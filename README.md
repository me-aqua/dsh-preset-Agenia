...
## 三份 markdown：人格、守则、实例库

内容**不在 YAML 里**，而在 `presets/agenia/` 下的三份 markdown：

| 文件 | 管什么 |
|---|---|
| `persona.md` | **她是谁** —— 名字、年龄、语气、性格 |
| `work-guidelines.md` | **怎么干活** —— 九条工程原则（纪律自动化、验证优先、反过度工程、别造轮子、最小改动、实测优先、代码与注释、测试、卡住先申请） |
| `codebase-practices.md` | **具体怎么做** —— 实例库：钩子分层、提交规范、lint 定位、注释规范、测试分层、真浏览器验证、多平台换行 |

后两份提炼自一个真实的重构：`me-aqua/TavernGame` 的 `refactor/vue-ts` 分支 ——
一位资深工程师把原生 JS 版本用 52 次提交重写成规范化工程（139 文件 / +21499-3394 行）。
**每一条都有出处**，不是凭空写的建议。

每发起一次模型请求，DSH 都会重新装配系统提示词，插件在那一刻重新读这三个文件，
把内容作为**每轮上下文**按顺序注入到**整个请求的最末尾**（在系统提示词之后、也在
项目自己的 `AGENTS.md` 之后）。放大模型注意力最强的地方，长对话里最不容易漂。

这意味着：

- **改任何一个 .md，下一个步骤就生效** —— 正在跑的会话也会变，不用重开会话、
  不用重启 harness。实测过：改文件后立刻重新装配，内容就换了。
- 按 mtime + 文件大小判断是否重读，所以编辑不会被忽略。
- 某个文件读不到时**不会让 agent 崩**：那一个源跳过，其余照常注入，并记一次日志。
- 想加第四份？在 `persona-plugin/inject.js` 的 `SOURCES` 数组里加一行就行。

## 改人格 / 改守则

**直接改那几个 .md 就行，保存即可生效。**

- **改名** —— 只改 `persona.md`。她的名字、年龄、性格、口头禅全在那一个文件里，
  整个仓库没有第二处。`agent.cordis.yml` 是**静态**的，里面一行人格内容都没有，
  所以改名不会漏改。
...
- **不要在任何一份 .md 里写半截的 `{{`。** 提示词里的 `{{...}}` 是变量引用，
  写错会直接抛错。
- **不要把项目专属的规矩写进这三份文件。** 它们是**全局**的，会跟着预设进每一个
  项目；项目自己的约定归项目自己的 `AGENTS.md`。
...
> **改完 `inject.js` 要么重启 harness，要么把数字加一**。
>
> **那三份 .md 都不受影响**——它们每轮从磁盘读，改完存盘照旧即时生效。
...
└── agenia/
    ...
    ├── persona.md             # ★ 她是谁（人格）
    ├── work-guidelines.md     # ★ 怎么干活（九条原则）
    ├── codebase-practices.md  # ★ 具体怎么做（实例库）
    ├── persona-plugin/
    │   ├── package.json       # 插件清单（name 和 version 都必须有，见 AGENTS.md）
    │   └── inject.js          # 每轮读上面三份 md，按 SOURCES 顺序注入到请求末尾
