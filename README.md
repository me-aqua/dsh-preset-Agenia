...
| **项目知识** | `AGENTS.md`、`AGENTS.local.md` | 各个项目自己的仓库 | 该项目的约定和规矩，由 `dsh-agent-instructions` 自动读进系统提示词 |

人格**故意写进预设**，因为她是同一个人、不该每个项目重写一遍。

人格行**不会**盖掉项目自己的 `AGENTS.md`：那是另一条独立的提示词段落，项目方
面照常生效。技术上的原因是 `persona` 那一行没有写 `complete: true` —— 一旦写了，
作用域内的这条人格就会变成*唯一*的提示词段落，项目文件也一起被顶掉。改那一行时
**不要加 `complete`**。
