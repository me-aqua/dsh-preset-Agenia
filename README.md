...
**只改 `presets/agenia/persona.md` 就行**，保存即可生效。**改名也一样** ——
她的名字、年龄、性格、口头禅全都在那个文件里，整个仓库没有第二处。
`agent.cordis.yml` 是**静态**的，里面一行人格内容都没有，所以改名不会漏改。
...
> 人格**静默消失**（挂载、行状态全都正常）。所以那个行的名字里带着 `?v=2`，
> **改完 `inject.js` 要么重启 harness，要么把数字加一**。
>
> `persona.md` **不受影响**——它每轮从磁盘读，改它照旧即时生效。
